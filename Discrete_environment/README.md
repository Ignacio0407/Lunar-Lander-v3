...
<p align="center">
  <img src="Presentation\media\lunar_lander.gif">
</p>
...
- [Deep Q-Network (DQN) for LunarLander-v3](#deep-q-network-dqn-for-lunarlander-v3)
  ...
  - [LunarLander-v3 ](#lunarlander-v3-environment)
...
- **Neural Network:** number_of_observations = 8 → 128 → 128 → number_of_actions = 4
- **Episodes:** 600
...
- **128_1**: `landed = (abs(pos_x) < 0.3 and pos_y < 0.3 and abs(vel_x) < 0.2 and abs(vel_y) < 0.2 and (leg1 == 1 and leg2 == 1))`. Knows where it has to land but always crashes because it does not lower speed (high speed in axis y).
- **128_2**: `landed = (abs(pos_x) < 0.1 and pos_y < 0.1 and abs(vel_x) < 0.1 and abs(vel_y) < 0.1 and (leg1 == 1 or leg2 == 1))`. Quite good model, mostly lands pretty well (rewards > 250) but crashes sometimes due to the high speed in axis y.
- **128_best**: `landed = (abs(pos_x) < 0.2 and pos_y < 0.2 and abs(vel_x) < 0.07 and abs(vel_y) < 0.03 and (leg1 == 1 or leg2 == 1) and abs(angle) < 0.2)`. Due to improved reward shaping, early stopping triggered in episode 376. Almost always lands perfectly, and when it does not surpass the environment (reward < 200) it scores around 150, unlike the 2nd one, which scored negatively.
- **128_hyperparameters_change**: Stopped training in episode 325. Hyperparameters: LR = 1.5e-4, EPSILON_MIN = 0.02, EPSILON_DECAY = 0.996, EARLY_STOPPING_THRESHOLD = 12, early_stopping_patience = 25. Sometimes lands correctly, sometimes keeps thrusting instead of landing.

**Verdict**: **128_best is the optimal model for standard environment**—excellent landing accuracy with efficient training.

### 256 Neurons Per Layer

- **256**: Early stopping triggered in episode 324. Results are pretty similar to "128_best" (landing accurately almost all times), meaning the task is so simple it does not require more neurons.
...
- **wind_3** (600 episodes and 3 hidden layers (1 more)): Clearly the best one so far, since it actually lands accurately sometimes between the flags with rewards around 240, but still it mostly drifts too far away, like model 1.
- **wind_4** (Equal to 3): Added 1 more hidden layer to the NN. Has almost learnt how and where to land, since it goes almost all times to the center, but many times does not land appropriately because it goes too fast, legs open and crashes.
- **wind_5** (Equal to 3): Similar to wind_4 with minor reward shaping differences. Had 50,000 buffer size in replay memory and batch size of 256. Results more or less the same as wind_4.
...
**Performance**: It is a bad model, but sometimes lands and sporadically lands well and tries to stick to the center.

**Verdict**: Fine-tuning from a highly optimized standard model to wind conditions was a bit worse than wind training from scratch, due surely to hyperparameters misconfiguration.
...
|----------|-------|----------|---------------|----------|
| **Standard Environment** | **128_best** | 376 | Excellent accuracy, fast convergence, efficient | **Demonstration** |
...
| **Worst Approach** | wind_1 | 320 | Complete failure in wind | Resource-constrained scenarios |
...
- **Fine-tuning was not as good as expected** when applied to wind conditions from the best standard model (128_best).
- Over-specialized standard models have a harder time adapting to environmental perturbations because their optimized policies become brittle.
...
3. **Complexity Ceiling**: Overly complex architectures degrade performance in simple control tasks. Match model capacity to environment complexity—bigger is not better.

4. **Wind Environment is Significantly Harder**: Even the best wind model (wind_3) only occasionally succeeds, while standard models succeed consistently. Wind requires substantially more training and better reward shaping.

5. **Early Stopping is Essential**: Prevents wasted computation and overfitting, with optimal stopping points between 300-400 episodes for well-shaped rewards.

6. **64 vs 128 vs 256 Neurons**: 
...
7. **Training Strategy Matters More Than Architecture**: The training approach (reward shaping, exploration strategy) has greater impact on success than network size or depth.
